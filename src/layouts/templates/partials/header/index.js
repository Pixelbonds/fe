import React, { useEffect, useState, useRef } from 'react';
import { CSSTransition } from 'react-transition-group';
import Link from 'next/link';
import { useOnClickOutside } from 'hooks';
import { siteUrl, categories } from 'config';
import { Button } from 'components';
import classNames from 'classnames';
import styles from './Header.module.scss';

export default function Header() {
  const [categoryVisible, setCategoryVisible] = useState(false);
  const categoryDropdownRef = useRef();

  useOnClickOutside(categoryDropdownRef, () => setCategoryVisible(false));

  return (
    <div className={styles.header}>
      <div className={styles.nav}>
        <Link href="/">
          <a className={styles.logo}>Pixelbonds</a>
        </Link>
        <span
          onClick={() => setCategoryVisible((current) => !current)}
          className={classNames(styles.navItem, {
            [styles.showDropdown]: categoryVisible,
          })}
        >
          <svg viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M12 16C13.1 16 14 16.9 14 18S13.1 20 12 20 10 19.1 10 18 10.9 16 12 16M12 10C13.1 10 14 10.9 14 12S13.1 14 12 14 10 13.1 10 12 10.9 10 12 10M12 4C13.1 4 14 4.9 14 6S13.1 8 12 8 10 7.1 10 6 10.9 4 12 4M6 16C7.1 16 8 16.9 8 18S7.1 20 6 20 4 19.1 4 18 4.9 16 6 16M6 10C7.1 10 8 10.9 8 12S7.1 14 6 14 4 13.1 4 12 4.9 10 6 10M6 4C7.1 4 8 4.9 8 6S7.1 8 6 8 4 7.1 4 6 4.9 4 6 4M18 16C19.1 16 20 16.9 20 18S19.1 20 18 20 16 19.1 16 18 16.9 16 18 16M18 10C19.1 10 20 10.9 20 12S19.1 14 18 14 16 13.1 16 12 16.9 10 18 10M18 4C19.1 4 20 4.9 20 6S19.1 8 18 8 16 7.1 16 6 16.9 4 18 4Z"
            />
          </svg>
          Category
          <CSSTransition
            in={categoryVisible}
            timeout={300}
            classNames={{
              // appear: 'my-appear',
              // appearActive: 'my-active-appear',
              // appearDone: 'my-done-appear',
              enter: styles.dropdownEnter,
              enterActive: styles.dropdownEnterActive,
              enterDone: styles.dropdownEnterDone,
              exit: styles.dropdownExit,
              exitActive: styles.dropdownExitActive,
              exitDone: styles.dropdownExitDone,
            }}
            unmountOnExit
          >
            <div className={styles.dropdown} ref={categoryDropdownRef}>
              <div className={styles.categoryGrid}>
                {categories.map((category, index) => (
                  <Link
                    key={index}
                    href={`${siteUrl}/explore/${category.slug}`}
                  >
                    <a>
                      <svg viewBox="0 0 24 24">
                        <path
                          fill="currentColor"
                          d="M12 16C13.1 16 14 16.9 14 18S13.1 20 12 20 10 19.1 10 18 10.9 16 12 16M12 10C13.1 10 14 10.9 14 12S13.1 14 12 14 10 13.1 10 12 10.9 10 12 10M12 4C13.1 4 14 4.9 14 6S13.1 8 12 8 10 7.1 10 6 10.9 4 12 4M6 16C7.1 16 8 16.9 8 18S7.1 20 6 20 4 19.1 4 18 4.9 16 6 16M6 10C7.1 10 8 10.9 8 12S7.1 14 6 14 4 13.1 4 12 4.9 10 6 10M6 4C7.1 4 8 4.9 8 6S7.1 8 6 8 4 7.1 4 6 4.9 4 6 4M18 16C19.1 16 20 16.9 20 18S19.1 20 18 20 16 19.1 16 18 16.9 16 18 16M18 10C19.1 10 20 10.9 20 12S19.1 14 18 14 16 13.1 16 12 16.9 10 18 10M18 4C19.1 4 20 4.9 20 6S19.1 8 18 8 16 7.1 16 6 16.9 4 18 4Z"
                        />
                      </svg>
                      <span>{category.name}</span>
                    </a>
                  </Link>
                ))}
              </div>
            </div>
          </CSSTransition>
        </span>
        <Link href="/explore">
          <span className={styles.navItem}>Explore</span>
        </Link>
      </div>
      <div className={styles.rightNav}>
        <Button bold>Submit</Button>
      </div>
    </div>
  );
}
